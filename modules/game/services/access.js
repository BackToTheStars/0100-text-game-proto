...
//
// Возвращает { gameId, hash, role, code, nickname, v } (code, nickname и v —
// только когда токен принят) или бросает ошибку с statusCode, текстом и машинным
...
const resolveGameAccess = async ({ hash, token, requireToken = false }) => {
  ...
  if (!token) {
    ...
    return { gameId, hash, role: ROLE_GAME_VISITOR };
  }
